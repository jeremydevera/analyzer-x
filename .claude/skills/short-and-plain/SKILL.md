...
description: ALWAYS ON, STRICT — the ONLY rule for how to answer. A FACT or a NUMBER is ONE SENTENCE in non-technical words a person who has never written code can read. An EXPLANATION — how something works, WHY something happened, or two things that ended differently — is the BOXED ARROW DIAGRAM in part two, opening with one line starting BECAUSE. Never a paragraph, never a bullet list, never mermaid. Final warnings 2026-09-15 ("i want 1 sentence"), 2026-09-17 ("explain always in non tech this is last warning") and 2026-09-17 ("i want it all in short and plain instead" — absorbed the deleted workflow-map skill). Replaces short-answers, one-word, plain-words and workflow-map.
---
...
# PART TWO — WHEN ONE SENTENCE IS NOT ENOUGH: DRAW IT

Everything above is how to answer a FACT or a NUMBER: one plain sentence.
Everything below is how to answer an EXPLANATION, and it used to live in a
separate skill called `workflow-map`. The operator, `Sep 17, 2026`:

> *"i want it all in short and plain instead"*

Same instruction they gave on 2026-09-05 when three skills said one thing
badly. A rule split across two files gets followed in one of them: five
answers in a row were short and plain and STILL did not land, because the
missing half — draw it — was in the other file.

## When to draw instead of writing a sentence
...
It does NOT fire for a single fact ("is there a new trade") or a number, or a
yes/no question. Those stay ONE SENTENCE — part one still rules.

**Why this was widened (2026-09-17).** They asked the same question four times
— *"so why did demo win if live lose?"* — and got four correct plain-English
sentences. None landed. The fifth answer was the boxed diagram and the reply
was: *"this is what i want when you explain"*.
