...
const F='file:///private/tmp/claude-501/-Users-jeremydevera-Desktop-Trading-Agents/e6e6f0ce-8b80-438a-80b9-9e9a91eba1f5/scratchpad/deploy-recommendation.html';
...
for (const theme of ['light','dark']) {
  ...
  console.log(theme,'cnt:',(await p.locator('#cnt').innerText()).slice(0,120));
  ...
  await p.fill('#margin','50'); await p.waitForTimeout(500);
  ...
  await p.selectOption('#siz','martingale'); await p.waitForTimeout(400);
  console.log(theme,'martingale foot:',(await p.locator('#foot td').nth(5).innerText()));
  await p.selectOption('#siz','flat'); await p.waitForTimeout(300);
  await p.screenshot({path:`/tmp/rec-${theme}.png`});
