...
| `remember-my-asks` | ALWAYS ON. Every prompt the operator types is appended verbatim to `docs/OPERATOR-ASKS.md` by a `UserPromptSubmit` hook (`.claude/hooks/record_ask.py`, wired in `.claude/settings.json`) — a HOOK, because a skill relies on Claude remembering, and in the REPO, because `~/.claude/.../memory/` is one machine and the ask was about another one. This skill is the instruction to READ it: before picking up work you did not start, when two rules conflict, when they say "i already told you". Quote it, never paraphrase. Asked for 2026-09-21. |
